...
Transform DREAM from a static format spec into an active dispatch system by creating a `dream-routing` skill with `assets/` subfolder (absorbing current `_templates/`), plus self-contained leaf skills (`dream-create-PP`, `dream-create-SP`, `dream-update`, `dream-close`, etc.) — each a complete SOP for a specific DREAM operation. Follows the same layered calling pattern as `orch-routing → orch-implementation`.
...
| p01_dream_routing/ | Plan | ⏳ [TODO] | Create dream-routing skill, move templates to assets/, implement dispatch |
...
p01_dream_routing/ ◄────────────────┘ (uses dispatch table)
        │
        ├──► dream-routing skill with assets/
        │
