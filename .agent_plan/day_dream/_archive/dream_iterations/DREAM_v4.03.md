# DREAM v4.03 — Unified Planning System Specification

**Version:** 4.03  
**Status:** ✅ Authoritative — supersedes v4.02  
...
**What changed from v4.02:** Typed plan folder prefixes (`SP01_`/`PP01_`), `_completed/` archival directory, `_tree.md` auto-generated folder tree, `_templates/` infrastructure rename, timestamp precision rules, `origin:` provenance field, Module Index table, State Delta gate condition, reconciliation checklist, `## Current Sprint` section, plan type tiebreaker, decision recording rule, `dream_mcp` enforcement module (P1).
...
8. [Deprecated (v4.02 → v4.03)](#deprecated-v402--v403)
...
| **Metadata** | YAML frontmatter in `_overview.md` (name, magnitude, status, origin, timestamps) |
...
| **Plan closure gate** | State Delta in root `_overview.md` required — plan CANNOT mark ✅ DONE without it |
| **Archival** | ✅ DONE / 🚫 CUT plans move to `_completed/` |
| **Tiebreaker** | Triggered by existing plan + modifies existing code → Procedure Plan |
...
│  • Gaps are allowed (SP01, SP03 — SP02 was cut or is procedure)  │
│  • Numbers reflect creation order across ALL plan types          │
│  • Visible when folders are collapsed in file explorer           │
│  • Applies to both active plans AND archived plans in _completed │
│                                                                   │
│  EXAMPLES:                                                        │
│    SP01_core_shop/        ← 1st plan created, System type        │
│    PP02_checkout_redesign/ ← 2nd plan created, Procedure type    │
│    SP03_marketplace/      ← 3rd plan created, System type        │
└───────────────────────────────────────────────────────────────────┘
```
...
├── _overview.md                         ← Root navigator + Current Sprint + Module Index + State Deltas
├── _tree.md                             ← Auto-generated folder tree (dream_mcp) — DO NOT HAND-EDIT
│
├── SP01_{plan_name}/                    ← 🔄 [WIP] System Plan — named plan directory
│   ├── _overview.md                     ← REQUIRED — plan navigator (frontmatter: name, magnitude, status, origin, timestamps)
...
│   │   └── {module_name}.md             ← Contains last_updated + modified_by_plans frontmatter
│   │
│   └── assets/                          ← Supporting artifacts
│       └── {id}_{desc}.asset.md
│
├── _completed/                          ← Archive of ✅ DONE and 🚫 CUT plans (moved here on closure)
│   ├── SP01_{plan_name}/                ← Retains prefix — never renamed
│   └── PP02_{plan_name}/
│
├── exploration/                         ← Research/exploration docs (decisions recorded here)
│   └── _archive/
...
.agent_plan/day_dream/
├── _overview.md                         ← Root navigator + Current Sprint + Module Index + State Deltas
├── _tree.md                             ← Auto-generated folder tree (dream_mcp)
│
├── PP03_{plan_name}/                    ← 🔄 [WIP] Procedure Plan — named plan directory
│   ├── _overview.md                     ← REQUIRED — plan navigator (frontmatter: name, magnitude, status, origin, timestamps)
...
|------|---------|---------------|
| `_overview.md` | Root navigator: Current Sprint, plan table, Module Index, State Deltas | Authors (human + agent) |
| `_tree.md` | Complete folder tree with inline status annotations | `dream_mcp` (auto-generated). Manual OK until `dream_mcp` exists |
| `_completed/` | Archive directory for ✅ DONE and 🚫 CUT plans | Authors move plans here on closure |
...
The `_tree.md` file is a **living folder tree** that provides a complete, annotated view of the day-dream directory.

```
┌────────────────────────────────────────────────────────────────────┐
│  _tree.md RULES                                                    │
│                                                                    │
│  Header:   <!-- DO NOT EDIT — regenerated by dream_mcp -->        │
│  Content:  Entire folder tree with inline annotations              │
│  Author:   dream_mcp (auto-generated via `dream tree`)            │
│  Edit:     NEVER hand-maintained (once dream_mcp exists)           │
│  Until:    dream_mcp is built, manual generation is acceptable     │
│  Refresh:  Regenerated on demand, not continuously                 │
└────────────────────────────────────────────────────────────────────┘
```
...
```markdown
<!-- DO NOT EDIT — regenerated by dream_mcp -->
...
├── _overview.md
├── _tree.md
├── SP01_core_shop/                      ← ✅ [DONE] System Plan
│   ├── _overview.md
│   ├── 01_executive_summary.md
│   ├── 02_architecture.md
│   ├── 03_product_catalog.md
│   └── 80_implementation.md
├── PP02_checkout_redesign/              ← ✅ [DONE] Procedure Plan
│   ├── _overview.md
│   ├── 01_summary.md
│   └── 80_implementation.md
├── SP03_marketplace/                    ← 🔄 [WIP] System Plan — NEW since last sprint
│   ├── _overview.md
│   ├── 01_executive_summary.md
...
Blueprint:
  └── SP01_{name}/                    ← Directory with structured files
      ├── _overview.md
...
│                                                                    │
│  checkout_redesign                                                 │
│    Triggered by: core_shop (existing plan)                        │
│    Primary deliverable: modifies existing checkout code            │
│    Result: PP (Procedure Plan) ✅                                  │
│                                                                    │
│  marketplace                                                       │
│    Triggered by: NOT an existing plan (new initiative)             │
│    Primary deliverable: creates new vendor_portal module           │
│    Result: SP (System Plan) ✅                                     │
└────────────────────────────────────────────────────────────────────┘
```
...
start_at: 2026-02-10                 # Optional — when work began (YYYY-MM-DD for human, ISO 8601 for dream_mcp)
last_updated: 2026-02-12             # MANDATORY — last modification date (YYYY-MM-DD for human, ISO 8601 for dream_mcp)
---
```
...
`start_at` is optional because some plans are exploratory — they exist before active work begins.
...
#### Root `_overview.md` — Extended Sections
...
- 🔄 SP03_marketplace/p01 — order splitting logic (Agent-B)
- ⏳ SP03_marketplace/p02 — payout system, commission engine
- 🔄 SP03_marketplace/05_order_routing.md — multi-vendor cart

## Plans

| Name | Type | Status | Description |
|------|------|--------|-------------|
| SP01_core_shop | System | ✅ [DONE] | Initial storefront |
| PP02_checkout_redesign | Procedure | ✅ [DONE] | State machine migration |
| SP03_marketplace | System | 🔄 [WIP] | Multi-vendor platform |

## Module Index

| Module | Origin Plan | Modified By |
|--------|------------|-------------|
| product_catalog | SP01_core_shop | PP04_multitenancy_migration |
| cart | SP01_core_shop | PP05_payment_migration |
| checkout | SP01_core_shop | PP02_checkout_redesign, PP05_payment_migration, PP04_multitenancy_migration |
| vendor_portal | SP03_marketplace | — |
...
| `## Plans` | After Current Sprint | All plans (active + completed) with type and status |
| `## Module Index` | After Plans | Maps each module to origin plan + modifier plans |
| `## State Deltas` | **LAST** section | Append-only changelog — what changed, when, by which plan |
...
---
module: checkout
last_updated: 2025-09-15        # MANDATORY — updated when any plan modifies this module
...
### 1.16 Level Annotation (Optional)
...
```
SP06_plugin_system/        ← L0 (root plan)
...
### 1.17 Executive Summary Requirements
...
### 1.22 Mandatory Skeleton Pattern (Documents)
...
### 1.26 Exploration Docs & Decision Recording
...
## Decisions
- ✅ CREATE: SP03_marketplace/ (System Plan, Epic)
- 🚫 CUT: SP01_core_shop/05_recommendation_engine.md
...
### 1.27 Clean-Code-First Directive
...
REPORT     ──► Mark plan status = ✅ [DONE]
               Notify parent
               Complete plan closure gates (see §2.11)
```
...
### 2.11 Plan Closure — Gates & Procedures

When marking a plan ✅ DONE or 🚫 CUT, the following gates **MUST** be satisfied:
...
│                                                                      │
│  ALL PLANS:                                                          │
│  ☐ All children marked ✅ DONE or 🚫 CUT                            │
│  ☐ State Delta appended to root _overview.md (GATE CONDITION)       │
│  ☐ Plan directory moved to _completed/                              │
│  ☐ last_updated frontmatter updated in plan's _overview.md          │
│                                                                      │
│  PROCEDURE PLANS (additional):                                       │
│  ☐ Reconciliation checklist completed in plan's _overview.md        │
│                                                                      │
│  ENFORCEMENT:                                                        │
│  dream_mcp validates these gates (when available). Until then,      │
│  agents self-enforce and human reviews on closure.                  │
└──────────────────────────────────────────────────────────────────────┘
```

#### State Delta (GATE CONDITION)

A plan **CANNOT** be marked ✅ DONE without appending a State Delta entry to the root `_overview.md`. State Deltas are **append-only** entries logging what changed in the codebase.
...
**Reading State Deltas tells you any module's full history in 30 seconds** — scan top-to-bottom for that module's name.

#### Reconciliation Checklist (Procedure Plans Only)

When a Procedure Plan is marked ✅ DONE, the author **MUST** complete a reconciliation checklist in the plan's `_overview.md`. This verifies that all touched module specs are updated.

```markdown
## Reconciliation
- [x] checkout.md (SP01_core_shop/modules/) — last_updated: 2025-09-15
- [x] inventory_sync.md (SP01_core_shop/modules/) — last_updated: 2025-09-15
- [x] cart.md (SP01_core_shop/modules/) — reviewed, no changes needed
```
...
1. Move the plan directory to `.agent_plan/day_dream/_completed/`
...
├── SP01_core_shop/    ← ✅ DONE       ├── SP03_marketplace/  ← 🔄 WIP (active)
├── SP03_marketplace/  ← 🔄 WIP       ├── _completed/
└── ...                                │   └── SP01_core_shop/ ← archived
                                       └── ...
```
...
### 2.14 `dream_mcp` — Enforcement Module

`dream_mcp` is the planned enforcement mechanism for DREAM conventions. It is a **P1 scope** module — referenced here for context, designed and built separately.

```
┌────────────────────────────────────────────────────────────────────┐
│  dream_mcp — P1 WALKING SKELETON                                   │
│                                                                    │
│  COMMANDS:                                                         │
│    dream status  — Reads frontmatter, prints plan status table     │
│    dream tree    — Generates _tree.md from folder structure         │
│                                                                    │
│  PROPERTIES:                                                       │
│    Read-only  — validates but NEVER mutates plan files             │
│    Reports    — surfaces staleness, gate violations, status        │
│                                                                    │
│  ENABLES:                                                          │
│    • Staleness detection (last_updated age)                        │
│    • Gate condition enforcement (State Delta check before DONE)    │
│    • Timestamp precision (machine-authored ISO 8601)               │
│    • _tree.md generation with inline annotations                   │
│    • Plan prefix validation (SP/PP + creation order)               │
│                                                                    │
│  UNTIL dream_mcp EXISTS:                                           │
│    Agents self-enforce all conventions.                            │
│    Manual _tree.md generation is acceptable.                       │
│    Human reviews plan closures for gate compliance.                │
└────────────────────────────────────────────────────────────────────┘
```
...
| Template | Purpose | Line Limit |
|----------|---------|------------|
| `blueprint/overview.template.md` | `_overview.md` scaffold — plan navigator (includes `origin:`, timestamps) | ≤100 lines |
...
| `blueprint/80_implementation.template.md` | Phased roadmap with verification (8-slot scale) | ≤200 lines/phase |
...
| `blueprint/modules/module_spec.template.md` | Detailed module implementation spec (includes `last_updated` frontmatter) | ≤200 lines |
...
| Don't | Do Instead |
|-------|------------|
| Plan folder without type prefix | Use `SP01_` or `PP01_` — always |
| Reassign plan numbers after creation | Numbers are IMMUTABLE — gaps are fine |
| Hand-edit `_tree.md` when `dream_mcp` exists | Let `dream_mcp` regenerate it |
| Keep completed plans in root | Move to `_completed/` on plan closure |
...
| Close plan without State Delta | State Delta is a GATE CONDITION — mandatory |
...
## Deprecated (v4.02 → v4.03)

| Change | Was (v4.02) | Now (v4.03) |
|--------|-------------|-------------|
| Plan folder naming | `{plan_name}/` (bare names) | `{TYPE}{NN}_{plan_name}/` — mandatory `SP01_`/`PP01_` prefix |
| Templates directory | `templates/` | `_templates/` — underscore prefix = infrastructure convention |
...
| Plan closure | Mark status ✅ DONE | Gate conditions: State Delta + archival to `_completed/` + (Procedure) reconciliation checklist |
...
| Plan type selection | No tiebreaker for edge cases | Tiebreaker: existing plan trigger + modifies existing code → Procedure |
...
| **Type-prefix folders** | `SP01_`/`PP01_` — plan type and creation order visible when collapsed |
| **Gate-enforced closure** | State Delta is mandatory — not a recommendation, a gate condition |
| **Archival over deletion** | Completed plans move to `_completed/`, never deleted |
| **Provenance tracking** | Every plan has an `origin:`, every module has `modified_by_plans:` |
| **Infrastructure underscore** | `_overview.md`, `_completed/`, `_templates/`, `_tree.md` — underscore prefix = infrastructure files |
...
| `dream-planning` skill | Update folder structure examples to show `SP01_`/`PP01_` prefixes, `_completed/`, `_tree.md` | HIGH |
...
| `overview.template.md` (root) | Add `## Current Sprint`, `## Module Index`, `## State Deltas` sections | HIGH |
| `module_spec.template.md` | Add `last_updated:` and `modified_by_plans:` to frontmatter scaffold | HIGH |
...
| Item | What to Change | Priority |
|------|---------------|----------|
| Create `dream_mcp` module spec | P1 — separate planning effort. Walking skeleton: `dream status` + `dream tree`. Read-only, validates but never mutates | P1 |

### Directory Rename

| Item | What to Change | Priority |
|------|---------------|----------|
| Rename `templates/` → `_templates/` | Physical directory rename in `.agent_plan/day_dream/`. 27+ references across skills, agents, and docs need updating | HIGH |

---

*End of DREAM v4.03 — Unified Planning System Specification*
