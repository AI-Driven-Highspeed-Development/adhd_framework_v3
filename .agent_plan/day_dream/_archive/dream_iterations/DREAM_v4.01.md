# DREAM v4.01 — Unified Planning System Specification
...
**DREAM** (Decomposition Rules for Engineering Atomic Modules) is a protocol for breaking complex work into atomic, parallelizable units that AI agents execute independently. It defines **how to assess complexity** (magnitude routing), **how to structure plans** (filesystem hierarchy with `_overview.md` convention), and **how agents coordinate** (sibling firewall + MANAGER/WORKER lifecycle). Metadata lives in `_overview.md` frontmatter — no YAML sidecar files.
...
2. [Chapter 1 — Route](#chapter-1--route)
3. [Chapter 2 — Structure](#chapter-2--structure)
4. [Chapter 3 — Author](#chapter-3--author)
5. [Chapter 4 — Execute & Verify](#chapter-4--execute--verify)
6. [Templates Reference](#templates-reference)
7. [Anti-Patterns](#anti-patterns)
8. [Deprecated (v3 → v4.01)](#deprecated-v3--v401)
...
| Concept | Rule |
|---------|------|
| **Magnitudes** | Trivial · Light · Standard · Heavy · Epic |
...
| **Levels** | L0–LN (N = nesting depth from root). Optional annotation for communication |
| **Phases** | `pNN_name/` directories inline in plan tree. Always directories, never files |
| **Bootstrap** | Enter dir → read `_overview.md` → check parent's `_overview.md` for boundaries → act |
...
| **Max nesting depth** | 3 levels recommended |
| **Walking skeleton** | Opt-in — required only when cross-boundary, external API, or 3+ module flow |
| **P0 hard limits** | 2–8h AI time, max 5 tasks, `[KNOWN]` only |
| **Acceptance criteria** | `## Acceptance Criteria` with `- [ ]` checklists in task files. No `.spec.md` files |
| **Estimation default** | AI-agent time. Mark `human_only: true` for human-judgment tasks |
| **Flatten rule** | Single-child plans **SHOULD** flatten, **except** phases (always directories) |
| **Dispatch** | Informal via orchestrator/skills. No YAML dispatch schema |

---

## Chapter 1 — Route

This chapter answers: *"Does this work need DREAM? If so, how deep?"*

### 1.1 Magnitude Assessment

Assess magnitude **first**. This is agent judgment, not a rigid checklist.

| Magnitude | Slots | Action | AI-Agent Time | Human Time (ref) |
|-----------|-------|--------|---------------|-------------------|
...
After magnitude, select a documentation tier:
...
Auto-detection triggers for Blueprint tier:
...
This chapter answers: *"How do I organize plans on disk?"*
...
A plan **MUST** have `_overview.md`. A task **MUST** be a leaf file with no children.

### 2.2 Metadata: `_overview.md` Frontmatter

All plan metadata lives in YAML frontmatter of `_overview.md`. **No separate YAML files** (`plan.yaml`, `node.yaml`, `contract.yaml` are all deprecated).
...
| 02_token_refresh.md | Task | 🔄 [WIP] | Token lifecycle |
...
2. 02_token_refresh.md (depends on login)
```

**Required `_overview.md` sections:**

| Section | Purpose |
|---------|---------|
| **Frontmatter** | `name`, `magnitude`, `status` |
| **Purpose** | Why this plan exists (2–3 sentences) |
| **Children** | Table of child plans/tasks with type, status, description |
| **Integration Map** | How children's outputs combine |
| **Reading Order** | Numbered list with dependency notes |

### 2.3 Level Annotation (Optional)

Levels are **L0–LN**, where N = nesting depth from root. These are optional communication aids, not enforced semantics.

```
project_plan/          ← L0 (root)
├── feature_auth/      ← L1
│   ├── oauth/         ← L2
│   │   └── task.md    ← L3 (task)
│   └── task.md        ← L2 (task)
...
Use level annotations when referring to plans in discussion (e.g., "the L1 auth plan"). Do **not** encode them in filenames or metadata.

### 2.4 Flatten Rule

A plan with only 1 child is suspicious — it is probably a task disguised as a plan. **SHOULD** flatten single-child plans into their parent.

**Exception:** Phases are always directories (see §2.5). This overrides the flatten rule for `pNN_name/` directories specifically, because sequential ordering **MUST** be preserved in file explorers.

### 2.5 Phase Directories

Phases use `pNN_name/` directories inline in the plan tree. No `phases/*.yaml` aggregation.
...
### 2.6 Blueprint Folder Structure
...
│   ├── _overview.md                ← Mandatory navigator (with frontmatter metadata)
...
### 3.2 Story Section Structure (Visual-First)
...
| Do This | Not This |
|---------|----------|
| ASCII boxes, tables, emoji anchors | Paragraphs of prose |
| 10-second grasp of problem/solution | Requires careful reading |
| Pain → Vision → One-Liner → Impact | Unstructured narrative |

### 3.3 Status Syntax

Use hybrid emoji + text markers everywhere. No plain-text alternatives.
...
| 🔄 `[WIP]` | In progress |
| ✅ `[DONE]` | Complete |
| 🚧 `[BLOCKED:reason]` | Stuck (use kebab-case reason, e.g., `[BLOCKED:waiting-on-api]`) |
| 🚫 `[CUT]` | Removed from scope |

### 3.4 Difficulty Labels

Every feature and task **MUST** carry a difficulty label:
...
Task files **MUST** include an `## Acceptance Criteria` section with checkbox lists. No separate `.spec.md` files.
...
| `implementation.md` | Blueprint tier | ≤200 lines per phase |
...
### 3.7 Executive Summary Requirements
...
### 3.8 Estimation Defaults

All durations use **AI-agent time** unless marked `human_only: true`.

| Magnitude | AI-Agent Time | Human Time (ref) |
|-----------|---------------|-------------------|
...
| Phase | Constraint |
|-------|-----------|
| **P0 (Walking Skeleton)** | Achievable in 1–2 weeks. Working passthrough/stub that proves plumbing. NO complex logic. Only `[KNOWN]` items |
...
### 3.12 Mandatory Skeleton Pattern (Documents)
...
**Diagram rules:** Use Mermaid for all supported chart types. Use ASCII art only when Mermaid does not support the format. Prefer SVG over PNG/JPG.

### 3.16 Exploration Docs
...
### 3.17 Clean-Code-First Directive
...
## Chapter 4 — Execute & Verify

This chapter answers: *"How do agents process plans and tasks?"*

### 4.1 Bootstrap Sequence

When an agent enters a plan directory, it **MUST** follow this sequence:
...
### 4.2 MANAGER Lifecycle (Processes a Plan)
...
### 4.3 WORKER Lifecycle (Fulfills a Task)
...
IMPLEMENT  ──► Read task spec, create/modify artifacts
               DO NOT modify sibling tasks or parent plan
                              │
                              ▼
VERIFY     ──► Check acceptance criteria (§3.5)
               Run verification steps (§3.10)
                              │
                              ▼
...
### 4.7 Updating Shared Files

For any request to update a file owned by a higher layer, the agent **MUST** report this to the parent. The parent decides how to proceed (e.g., wait for all siblings to complete, then update).

### 4.8 Subagent Dispatch
...
| `blueprint/01_executive_summary.template.md` | Vision, goals, non-goals, prior art | ≤150 lines |
...
| Complexity Classes A–D | Human review gates (Surface→Dense) | Replaced by difficulty labels `[KNOWN]`/`[EXPERIMENTAL]`/`[RESEARCH]` |
...
*End of DREAM v4.01 — Unified Planning System Specification*
