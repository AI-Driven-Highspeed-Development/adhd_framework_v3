...
-   **Plan Landscape**: Call `dream_status` and `dream_tree` to get the current plan dashboard and annotated hierarchy before creating or modifying plans.
...
-   **Use Templates**: Copy templates from `.github/skills/dream-routing/assets/` as starting points. NEVER edit the template files directly. See `.github/skills/dream-routing/assets/examples/` for completed samples.
-   **FREE ZONE**: Use `## [Custom] 🎨 Title` for project-specific sections (max 5). See `.github/skills/dream-routing/assets/examples/free_zone_*.example.md`.
-   **Deep Dive**: Add `## 🔬 Deep Dive` only when algorithms, API contracts, or error handling need explicit design. See `.github/skills/dream-routing/assets/examples/deep_dive_*.example.md`.
...
-   **Plan Validation**: After creating or updating a plan, call `dream_validate` to verify structural correctness. Before modifying a plan with dependencies, call `dream_impact` to assess downstream effects.
